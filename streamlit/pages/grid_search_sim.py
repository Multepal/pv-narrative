"""
Parameter Grid Search — Cosine-Similarity HAC (ARI) — same as grid_search_pca.py
but replaces TruncatedSVD with a cosine similarity matrix.

TF-IDF → cosine similarity (n_chunks × n_chunks) → Ward HAC.
k is swept cheaply post-linkage (fcluster only). Results are cached.
"""
...
st.title("Parameter Grid Search — Cosine-Similarity HAC (ARI)")
render_toc([
    ("ARI vs. k",              "ari-chart"),
...
st.caption(
    "Sweeps combinations of `n_chunks` and `max_df`. For each combination, TF-IDF is computed "
    "then converted to an n_chunks × n_chunks cosine similarity matrix; Ward HAC is run once "
    "per edition, then k is swept from 2–20 cheaply via fcluster. "
