...
def render_hac_dendrogram(result, THETA, n_actual, lsa_key, selected_k, selected_n, src_id, n_tokens_early):
    """Renders k-selection scree + rotated dendrogram. Returns cluster_state dict."""
    ...
    fig_hac.add_vline(
        x=selected_k, line_dash="dash", line_color="crimson", line_width=1.5,
        annotation_text=f"k = {selected_k}", annotation_position="top right",
    )
    ...
    _hac_evt = st.plotly_chart(fig_hac, width='stretch', key=f"lsa_hac_{lsa_key}_{selected_n}", on_select="rerun")
    ...
    _threshold  = threshold_for_k(_Z, selected_k, n_actual)
    ...
    _key_sfx    = f"{lsa_key}_{selected_n}_{selected_k}"
    ...
    st.caption(
        ...
        f"{n_tokens_early:,} tokens · {n_actual} chunks · "
        f"{_n_clusters} clusters · {selected_n} PCA components"
    )

    return {"Z": _Z, "labels": _labels, "c2color": c2color, "unique": _unique,
...
def render_component_scatter(THETA, components, words, result, cs, selected_n):
    ...
    _labels, c2color, _unique = cs["labels"], cs["c2color"], cs["unique"]
    _n_clusters, _key_sfx = cs["n_clusters"], cs["key_sfx"]

    if selected_n >= 2:
        _pc_options  = [f"PC{i + 1}" for i in range(selected_n - 1)]
...
def render_pca_hac_agreement(THETA, result, cs, n_actual):
    ...
    _Z, _labels, c2color, _unique = cs["Z"], cs["labels"], cs["c2color"], cs["unique"]
    _n_clusters, _key_sfx = cs["n_clusters"], cs["key_sfx"]
...
def render_cluster_profiles(THETA, components, words, cs, selected_n):
    ...
    _labels, _unique = cs["labels"], cs["unique"]
    _n_clusters, _key_sfx = cs["n_clusters"], cs["key_sfx"]

    _cluster_labels_alpha = [chr(65 + i) for i in range(_n_clusters)]
    _centroid = np.array([THETA[_labels == _c].mean(axis=0) for _c in _unique])

    _hover_n  = 5
    _pos_pole = []
    _neg_pole = []
    for _ci in range(selected_n):
        _cv = components[_ci]
        _si = np.argsort(_cv)
    ...
    _cdata = np.empty((_n_clusters, selected_n, 2), dtype=object)
    for _ci in range(selected_n):
    ...
    fig_centroid = px.imshow(
        _centroid,
        x=[f"PC{i + 1}" for i in range(selected_n)],
        y=_cluster_labels_alpha,
        aspect="auto",
        color_continuous_scale=_v["colorscale_pca"],
...
def render_component_poles(components, words, selected_n):
    ...
    for comp_idx in range(selected_n):
...
def render_variance_explained(explained, cum_var, selected_n, result, lsa_key):
    ...
    fig_scree.add_trace(go.Scatter(
        x=_comp_vals, y=cum_var * 100,
        name="Cumulative", line=dict(color="#EF553B"), mode="lines+markers", showlegend=True,
    ))
    fig_scree.add_vline(
        x=selected_n, line_dash="dash", line_color="crimson", line_width=1.5,
        annotation_text=f"n = {selected_n}", annotation_position="top right",
    )
    ...
    _scree_evt = st.plotly_chart(fig_scree, width='stretch', key=f"lsa_scree_{lsa_key}", on_select="rerun")
    if _scree_evt.selection.points:
        ...
        if _clicked_n != st.session_state.get("lsa_n_components") and 1 <= _clicked_n <= result['n_components']:
            ...
            st.rerun()
    st.success(f"n_components = {selected_n} · {cum_var[selected_n - 1] * 100:.1f}% variance explained")


def render_component_scores(THETA, selected_n, n_actual, lsa_key):
    ...
    fig_heat = px.imshow(
        THETA.T,
        y=[f"PC{i + 1}" for i in range(selected_n)],
        x=_x_pos,
        aspect="auto",
        color_continuous_scale=_v["colorscale_pca"],
        color_continuous_midpoint=0.0,
    ...
    fig_heat.update_layout(
        height=max(180, selected_n * cfg["layout"]["heatmap_row_height_px"] + 60),
        margin=dict(l=60, r=20, t=20, b=40),
    )
...
st.divider()
render_pca_hac_agreement(THETA, result, cs, n_actual)

render_cluster_profiles(THETA, components, words, cs, _selected_n)

st.divider()
render_component_poles(components, words, _selected_n)

st.divider()
render_variance_explained(explained, _cum_var, _selected_n, result, _lsa_key)

st.divider()
render_component_scores(THETA, _selected_n, n_actual, _lsa_key)
