"""
Cross-Edition Cluster Comparison (NMF) — TF-IDF → NMF → argmax(THETA) labels,
compared across editions using Hamming distance and ARI.
"""
...
from utils import (find_token_file, load_tokens, cluster_string,
...
HOLDOUT_DEFS = {
    "k":         {"label": "k (topics)",  "dtype": int,   "vals": list(range(2, 21))},
...
def run_nmf(src_id, token_path, n_chunks, min_df, max_df, k, ngram_range=(1, 1)):
    ...
    return {"labels": labels, "n_chunks": len(chunks_list)}


# ── Controls ──────────────────────────────────────────────────────────────────
st.title("Cross-Edition Comparison — NMF")
render_toc([
    ("Agreement vs. Parameter",    "agreement-chart"),
    ("Pairwise Distance Matrices", "pairwise-matrices"),
])
st.caption(
    "Each edition is divided into `n_chunks` equal bins. TF-IDF → NMF is fit per edition; "
    "chunk labels are the dominant topic (argmax of THETA). "
    "Both **Hamming** and **ARI** are shown as higher = more agreement."
)
...
for step_i, val in enumerate(sweep_vals):
    ...
    for src_id, meta in SOURCES_META.items():
        ...
        if token_path is None:
            continue
        TOKEN  = load_tokens(src_id, token_path)
        result = run_nmf(src_id, token_path, p_n_chunks, p_min_df, p_max_df, p_k,
