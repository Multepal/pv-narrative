"""
Parameter Grid Search — Boundary Concordance (LSA/HAC) — sweep combinations of
n_chunks and max_df, measuring how consistently narrative boundaries (cluster
...
to a set of normalized boundary positions in [0, 1] (the relative positions
where the cluster label changes). Pairwise boundary F1 within a tolerance of
one chunk width is averaged across all edition pairs.

Unlike ARI, this metric rewards editions that agree on *where* narrative
transitions occur, not whether the exact same chunk indices share a label.

k is swept cheaply post-linkage (fcluster only). SVD+linkage runs are cached.
"""
...
def run_linkage(src_id, token_path, n_chunks, min_df, max_df, ngram_range=(1, 1)):
    ...
    if n_comp < 2:
        return None
    X_lsa = TruncatedSVD(n_components=n_comp, random_state=42).fit_transform(X)
    Z = linkage(pdist(X_lsa, metric="euclidean"), method="ward")
...
st.title("Grid Search — Boundary Concordance (LSA)")
render_toc([
...
st.caption(
    f"TF-IDF → LSA (TruncatedSVD, n_components={N_COMPONENTS}) → Ward HAC. "
    "For each (n_chunks, max_df, k), each edition's cluster assignment is converted "
    ...
    "k is swept cheaply post-linkage; SVD+linkage results are cached."
)
...
st.caption(
    f"**{n_combos}** combinations · **{n_combos * len(SOURCES_META)}** SVD+linkage runs (est.)  "
    f"·  min_df={FIXED_MIN_DF} · ngram=(1,1) · n_components={N_COMPONENTS} fixed"
)
...
for ci, (nc, mxdf) in enumerate(combos):
    ...
    for k in K_VALS:
        tol = 1.0 / (2 * max(k - 1, 1))  # half the expected inter-boundary gap; stricter as k grows
        label_arrays = []
