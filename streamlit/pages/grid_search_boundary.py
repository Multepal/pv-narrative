"""
Parameter Grid Search — Boundary Concordance (LSA/HAC)
"""
...
from grid_search_boundary_core import (
    find_token_file, load_tokens, run_linkage_lsa,
...
for ci, (nc, mxdf) in enumerate(combos):
    ...
    for src_id in SOURCES_META:
        ...
        if tp is None:
            continue
        result = run_linkage_lsa(src_id, tp, nc, FIXED_MIN_DF, mxdf, FIXED_NGRAM)
...
st.caption(
    f"k=2 excluded — F1 peaks trivially at minimum k. "
    f"Genuine maximum at k={_k_star}."
)
