"""
Parameter Grid Search — Boundary Concordance (Cosine-Sim/HAC)
"""
...
from grid_search_boundary_core import (
    find_token_file, load_tokens, run_linkage_sim,
...
for ci, (nc, mxdf) in enumerate(combos):
    ...
    for src_id in SOURCES_META:
        ...
        if tp is None:
            continue
        result = run_linkage_sim(src_id, tp, nc, FIXED_MIN_DF, mxdf, FIXED_NGRAM)
