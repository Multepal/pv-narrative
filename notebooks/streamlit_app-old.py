...
def find_token_file(src_id: str) -> str | None:
    """
    Search common relative locations for the TOKEN CSV.
    Returns the path if found, else None.
    """
    candidates = [
        f"../{src_id}/{src_id}-TOKEN.csv",
        f"{src_id}/{src_id}-TOKEN.csv",
        f"./{src_id}-TOKEN.csv",
    ]
    for p in candidates:
...
def chunk_tokens(TOKEN: pd.DataFrame, chunk_size: int, overlap: int, min_len: int = 50):
    """Split token list into overlapping word-level chunks."""
...
with st.sidebar:
    ...
    # Source selector with language grouping
    ...
    overlap_pct = st.slider("Overlap", 0.0, 0.9, 0.9, step=0.01,
                            format="%.2f")
...
if token_path is None:
    st.info(
        f"""
**Token file not found for `{src_id}`.**

The app expects a CSV at one of these relative paths:
- `../{src_id}/{src_id}-TOKEN.csv`
- `{src_id}/{src_id}-TOKEN.csv`
- `./{src_id}-TOKEN.csv`

You can also upload the file directly below.
""",
        icon="📂",
    )
...
# ── Run on button press (or first load if path was found) ─────────────────────
if run_btn or ("theta" not in st.session_state):
    ...
    if len(chunks) < 2:
        st.warning("Too few chunks produced — try a smaller chunk size or less overlap.")
        st.stop()
...
if "theta" in st.session_state:
    ...
    st.caption(
        ...
        f"X-axis = syntagm (narrative position) · Y-axis = paradigm (topic structure)"
    )
    ...
    # Colour columns for quick scanning
    st.dataframe(
        topic_display,
        use_container_width=True,
        height=min(40 + 35 * len(TOPICS), 500),
    )

    # Download
    csv_bytes = THETA.to_csv().encode()
    st.download_button(
