...
Because of its unique status, the Popol Wuj has long been regarded by scholars as a “window into the mind of the Ancient Maya” (Christenson and Sachse 2021: 3), that provides a view relatively untainted by the beliefs and practices brutally imposed by their Spanish conquerors. Following the lead of Michael Coe (1973), the text has served as a key to the interpretation of numerous scenes depicted on carved and painted artifacts from the lowland preclassic and classic periods. Specific characters and episodes from the story have been identified on items dated to 300 BCE, two millennia before the composition of the current text.
...
-   Choice of chunk: Avoid editorial bias, Neutral, use overlap, compare to number of paragraphs in original

-   Choice of chunk size: Heap's Law? Or, roughly mean paragraph size

-   Choice of min_df: roughly equal to number of topics

-   Choice of max_df: $.5$

-   Choice of k: Mean UMass Topic Coherence (Mimno), compare to Temas dot plot

-   Chunk overlap

    <div>

    Following Hearst (1997), we segment the text using overlapping sliding windows of width $W$, which implement a rectangular kernel smoother of bandwidth $W$ over the term-occurrence signals. The step size $s = W(1−ρ)$ controls sampling density; smaller steps produce denser, smoother topic trajectories without altering the averaging bandwidth.
